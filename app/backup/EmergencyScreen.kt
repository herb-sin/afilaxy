...
fun EmergencyScreen(
    ...
    LaunchedEffect(Unit) {
        try {
            ...
            val hasLocationPermission = hasFineLocationPermission || hasCoarseLocationPermission
            android.util.Log.d("EmergencyScreen", "Permissões: fine=$hasFineLocationPermission, coarse=$hasCoarseLocationPermission, total=$hasLocationPermission")
            viewModel.updateLocationPermission(hasLocationPermission)
        } catch (e: Exception) {
            android.util.Log.e("EmergencyScreen", "Erro ao verificar permissões: ${e.message}")
            viewModel.updateLocationPermission(false)
        }
    }

    // Permission launcher simplificado
    val requestPermissionLauncher =
            rememberLauncherForActivityResult(
                    contract = ActivityResultContracts.RequestPermission(),
                    onResult = { isGranted ->
                        android.util.Log.d("EmergencyScreen", "Permissão concedida: $isGranted")
    ...
    Column(
        ...
        when {
            uiState.hasLocationPermission -> {
                    ...
                    uiState.userLocation != null -> {
                        ...
                        when {
                            uiState.isAwaitingHelperResponse -> {
                                ...
                                Spacer(modifier = Modifier.height(16.dp))
                                Text("Ajudantes encontrados: ${uiState.nearbyHelpers.size}")
                                // Mostrar apenas os 3 primeiros para evitar ANRs
                                val limitedHelpers = uiState.nearbyHelpers.take(3)
                                Column {
                                    limitedHelpers.forEach { helper ->
                                        HelperCard(helper = helper)
                                    }
                                    if (uiState.nearbyHelpers.size > 3) {
                                        Text(
                                            text = "+ ${uiState.nearbyHelpers.size - 3} ajudantes adicionais",
                ...
                Button(
                        onClick = {
                            android.util.Log.d("EmergencyScreen", "Solicitando permissão de localização")
                            try {
                                requestPermissionLauncher.launch(Manifest.permission.ACCESS_FINE_LOCATION)
                            } catch (e: Exception) {
                                android.util.Log.e("EmergencyScreen", "Erro ao solicitar permissão: ${e.message}")
                            }
                        }
...
private fun fetchCurrentUserLocation(context: Context, viewModel: EmergencyViewModel) {
    ...
    android.util.Log.d("EmergencyScreen", "Iniciando busca de localização para emergência")
